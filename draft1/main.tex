...
\begin{table}[H]
  \caption{Dataset Characteristics per Session. Note: the 36\degree{} afternoon session
           ...
           irradiance rather than treating all time intervals equally.}
  \label{tab:dataset}
  \centering
  \renewcommand{\arraystretch}{1.2}
  \begin{tabular}{@{}lrrl@{}}
...
13\degree{} counterpart (mean corrected power 0.0127 vs.\ 0.0101~W), consistent with the
geometric argument that steeper inclination better accommodates off-meridian solar paths.
...
consistency of the 36\degree{} advantage across all three time windows.
...
throughout the session, confirming that the steeper inclination maintains adequate
geometric alignment with the high-elevation June sun across the full transit window.
Third, the normalised efficiency figures (Figs.~\ref{fig:e13m} and~\ref{fig:e36mid})
confirm that the efficiency advantage of 36\degree{} persists even after irradiance
variability has been removed by the $\eta_{\text{rel}}$ normalisation, ruling out
differing atmospheric conditions as an explanation for the observed yield gap.
...
\begin{enumerate}
  \item \textbf{Non-simultaneous comparison:} The two angles were measured on consecutive,
        not simultaneous, days, meaning that day-to-day atmospheric variability cannot be
        fully excluded as a confound. However, the irradiance-normalisation framework of
        Eq.~(\ref{eq:noreff}) specifically mitigates---though does not eliminate---this
        confound: by dividing temperature-corrected power by ambient lux at each sample,
        first-order atmospheric variability (cloud cover, aerosol loading) is removed from
        the efficiency metric. Critically, the 36\degree{} measurement day experienced
        \textit{lower} mean ambient illuminance (20{,}068 lux vs.\ 30{,}172 lux on the
        13\degree{} day), yet the 36\degree{} configuration still outperformed by 45.86\%.
        This means the result is \textit{conservative} rather than inflated: if anything,
        the disadvantage in available irradiance on the 36\degree{} day argues against
        the observed superiority being an atmospheric artefact. Future work should
        nevertheless employ side-by-side simultaneous logging to fully eliminate this
        limitation.
...
Pending such multi-angle and multi-season validation, these findings offer a cautious but
concrete preliminary recommendation for practitioners: rooftop PV installations in Bengaluru
and similar tropical latitudes (12\degree{}--15\degree{}N) should consider tilt angles in the
30\degree{}--40\degree{} range during the May--July summer season rather than defaulting to the
conventional latitude-matched angle. This recommendation is supported by a measured 45.86\%
energy yield advantage at 36\degree{} over 13\degree{} under field conditions, and by the
irradiance-normalised efficiency analysis which demonstrates that this advantage is geometric
in nature and not an atmospheric artefact. Installers adopting a seasonally adjusted tilt
strategy---switching from a summer-optimised steep angle to a shallower winter angle---may
achieve meaningful improvement in annual energy yield without additional hardware cost.
