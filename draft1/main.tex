...
This assumption holds for the photopic-to-radiometric conversion factor used in the
normalisation ratio itself; it does not imply that the beam-to-diffuse composition of
irradiance was identical on both days. As discussed in Section~\ref{sec:limitations},
day-to-day differences in cloud cover or aerosol loading could still alter the diffuse
fraction of incident light even when the broad spectral character of sunlight is otherwise
similar, and this is treated as a limitation of the present design rather than a resolved
confound.
...
midday power profile is flatter and more sustained, which is consistent with the steeper
inclination maintaining closer alignment with the June sun's north-of-zenith arc at this
latitude, though atmospheric differences between the two measurement days cannot be ruled
...
\begin{figure}[H]
  ...
  \caption{Power vs.\ time, 36\degree{} midday session. The comparatively flat
           curve is consistent with sustained geometric alignment during the solar
           transit window.}
  \label{fig:p36mid}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Normalised efficiency $\eta_{\text{rel}}$, 36\degree{} midday session.
           The consistently elevated values relative to Fig.~\ref{fig:e13m} are
           consistent with the efficiency gap persisting after correcting for overall
           irradiance level, though as noted in Section~\ref{sec:limitations} this
           correction cannot account for beam-to-diffuse composition differences
           between the two measurement days.}
  \label{fig:e36mid}
\end{figure}
